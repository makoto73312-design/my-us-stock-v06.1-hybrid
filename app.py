...
# --- 1. 網頁核心外觀配置 ---
st.set_page_config(page_title="美股雷達 V06.1", page_icon="🔮", layout="wide")
st.title("🔮 美股量化沙盒 V06.1 (法人級多因子雙向感知與產業分流版)")
st.markdown("已實裝 **V06.1 四重法人防衛網**：**VIX恐慌感知**、**基本面護城河 (FCF/PE/營收)**、**財報避險盾**、**產業屬性標籤** 與 **雲端雙向自選股寫入**")
...
# 🟢 去重機制：自動過濾重複輸入的股票代號，並保留原本排名的先後順序
...
def fetch_fundamental_and_news(ticker, cloud_dict):
    ...
    try:
        ...
        # 判斷產業領域 (優先採用雲端自訂標籤 -> 美股資料庫對照 -> Yahoo Finance 官方Sector)
...
# --- 8. 網頁三分頁系統 ---
tab_summary, tab_manage, tab_debug = st.tabs(["📊 V06.1 美股多因子混合矩陣", "➕ 自選清單線上管理", "🔍 深度數據與視覺化面板"])
...
with tab_summary:
    ...
    if st.button("🚀 啟動 V06.1 美股全自動多因子掃描引擎", use_container_width=True):
        with st.spinner("正在進行總經、基本面、新聞與技術面全維度運算..."):
            ...
            for ticker in ticker_list:
                ...
                fund_info = fetch_fundamental_and_news(ticker, cloud_names_dict)
                
                for strat in strategies:
                    radar, ret, win, trades, pf, stars, cur_status, entry_price_val, sl_price, pnl, t_logs, p_buys, p_sells, v_df = run_backtest_engine(df_stock, strat, backtest_days, market_posture, fund_info)
            ...
            st.success("📊 V06.1 多因子矩陣計算完成！")

    if st.session_state.calculated:
        df_res = st.session_state.final_df
        
        # 🟢 倉位狀態分類看板 (Quick Classification Dashboard)
        ...
        with st.expander(f"📦 獲利續抱中 ({len(hold_df)} 筆)", expanded=False):
            if not hold_df.empty:
                st.dataframe(hold_df[display_cols], use_container_width=True, hide_index=True)
            else:
                st.info("目前無持有中之個股。")
        ...
        with st.expander(f"💵 空手觀望 ({len(cash_df)} 筆)", expanded=False):
            if not cash_df.empty:
                st.dataframe(cash_df[display_cols], use_container_width=True, hide_index=True)

        st.divider()
        st.subheader("📋 完整美股多因子對照矩陣")
...
with tab_debug:
    ...
    if st.session_state.calculated:
        ...
        if db_key in st.session_state.detail_db:
            ...
            fig.update_layout(title=f"<b>{debug_ticker} - {debug_strat} V06.1 軌跡圖</b>", xaxis_title="日期", yaxis_title="價格 ($)", template="plotly_dark")
